    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4 flex-1">
            <div class="relative flex-1 max-w-md">
                ...
                <form id="searchForm" method="GET" action="{{ route('inventory.brands') }}">
                    @if (request('status'))
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('inventory.brands', array_merge(request()->query(), ['status' => 'active'])) }}"
                ...
                <a href="{{ route('inventory.brands', array_merge(request()->query(), ['status' => 'inactive'])) }}"
                    class="px-4 py-2 rounded-lg text-sm font-medium transition-colors border
        ...
        <button onclick="openBrandModal('{{ route('inventory.brands.store') }}', '', '', '', '', 'active', false)"
    ...
    @if ($brands->isEmpty())
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="flex flex-col items-center justify-center py-16 px-6">
                ...
                <h3 class="text-base font-semibold text-gray-900 mb-1">No brands found</h3>
                <p class="text-sm text-gray-500 mb-6 text-center max-w-sm">
                    @if (request('search') || request('status'))
                        No brands match your current filters. Try adjusting your search or
                        <a href="{{ route('inventory.brands') }}"
                            class="text-indigo-600 hover:text-indigo-700 font-medium">clear filters</a>.
                    @else
                        Get started by creating your first brand to organize your inventory.
                    @endif
                </p>
                @if (!request('search') && !request('status'))
                    <button onclick="openBrandModal('{{ route('inventory.brands.store') }}', '', '', '', '', 'active', false)"
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100">
                            ...
                            <th class="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Brand
                            </th>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            @if ($brand->website)
                                                <p class="text-xs text-gray-500">{{ $brand->website }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <button type="button"
                                            onclick='openBrandModal("{{ route('inventory.brands.update', $brand) }}", "{{ $brand->name }}", "{{ $brand->slug }}", "{{ $brand->description }}", "{{ $brand->website }}", "{{ $brand->status }}", true)'
                                        ...
                                        <button type="button"
                                            onclick="confirmModal.open(function() { document.getElementById('delete-brand-{{ $brand->id }}').submit(); })"
                                        ...
                                        <form id="delete-brand-{{ $brand->id }}"
                                            action="{{ route('inventory.brands.destroy', $brand) }}" method="POST"
                                            class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
...
@section('modals')
    <x-confirm-modal id="confirmModal" title="Delete Brand"
        message="Are you sure you want to delete this brand? This action cannot be undone." confirmText="Delete"
        cancelText="Cancel" type="danger" />

    <x-confirm-modal id="bulkConfirmModal" title="Delete Brands"
        message="Are you sure you want to delete the selected brands? This action cannot be undone."
        confirmText="Delete All" cancelText="Cancel" type="danger" />

    <form id="bulkDeleteForm" action="{{ route('inventory.brands.bulkDelete') }}" method="POST" class="hidden">
        @csrf
    ...
    <div id="brandModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm transition-opacity" onclick="closeBrandModal()"></div>
        <form id="brandForm" class="relative bg-white rounded-2xl shadow-xl w-full max-w-md transform transition-all"
            method="POST">
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <div>
                    <h3 id="brandModalTitle" class="text-base font-semibold text-gray-900">Add New Brand</h3>
                    <p id="brandModalDesc" class="text-xs text-gray-500 mt-0.5">Create a new product brand.</p>
                </div>
            ...
            <div class="px-5 py-5 space-y-4">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1.5">Brand Name <span
                            class="text-red-500">*</span></label>
                    <input type="text" id="name" name="name" placeholder="e.g. Nike"
                ...
                <div>
                    <label for="slug" class="block text-sm font-medium text-gray-700 mb-1.5">Slug <span
                            class="text-red-500">*</span></label>
                    <input type="text" id="slug" name="slug" placeholder="e.g. nike"
                ...
                <div>
                    ...
                    <input type="text" id="website" name="website" placeholder="e.g. https://nike.com"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors bg-white">
                    @error('website')
                        <p class="mt-1 text-sm text-red-600">
                            {{ $message }}
            ...
            <div class="px-5 py-3 border-t border-gray-100 bg-gray-50 flex items-center justify-end gap-3 rounded-b-2xl">
                ...
                <button id="brandModalBtn" type="submit"
...
@section('footer-scripts')
    @if ($errors->any())
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                openBrandModal('{{ route('inventory.brands.store') }}', '{{ old('name') }}', '{{ old('slug') }}', '{{ old('description') }}', '{{ old('website') }}', '{{ old('status', 'active') }}', false);
            });
        </script>
    @endif
    ...
    <script>
        function openBrandModal(action, name, slug, description, website, status, isEdit) {
            ...
            if (isEdit) {
                ...
                document.getElementById('brandModalDesc').textContent = 'Update brand details.';
                document.getElementById('brandModalBtn').textContent = 'Update';
            } else {
                form.querySelector('[name="_method"]').value = 'POST';
                document.getElementById('brandModalTitle').textContent = 'Add New Brand';
                document.getElementById('brandModalDesc').textContent = 'Create a new product brand.';
                document.getElementById('brandModalBtn').textContent = 'Create';
            }
        ...
        document.addEventListener('keydown', function (e) {
            if (!document.getElementById('brandModal').classList.contains('hidden') && e.key === 'Escape') closeBrandModal();
        });
    </script>
@endsection
